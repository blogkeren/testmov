        ...
        $('article, .item, .post, .movie-item').each((i, element) => {
            try {
                ...
                if (image && !image.startsWith('http')) {
                    image = image.startsWith('//') ? 'https:' + image : config.kitanontonBaseUrl + image;
                }
...
export async function scrapeMovieDetail(contentUrl) {
    try {
        ...
        // PENTING: Logika Ekstraksi Server (Base64)
        const embedUrls = [];
        
        // 1. Cari elemen .server dengan data-iframe (Ini yang dipakai KN & Rebahin)
        $('.server').each((i, el) => {
            ...
            if (b64) {
                try {
                    ...
                    console.error('Gagal decode base64:', e.message);
                }
            }
        });

        // 2. Fallback: Cari Iframe langsung (jika ada yang tidak dienkripsi)
        ...
        return {
            ...
            description,
            poster,
            embedUrls, // Kirim URL hasil decode ke Router
